...
export default function VotingEscrowVault(props: {
  ...
  const { setDepositModal, setWithdrawModal, setQuitLockModal, setExtendLockModal } = props;
  ...
  return (
    <VaultCard vaultTitle="VeVault">
      <DetailCard label="Balance" data={`${formatNumberToFixed(balance)} FDT`} />
      <DetailCard label="Lock Expiration" data={lockEndDate || '-'} />
      <DetailCard label="Voting Power" data={`${formatNumberToFixed(Number(votingPower))} FDT`} />
      <div className="flex w-full">
        <div className="flex flex-col mr-4 w-full">
          <ActionCard onClick={() => setDepositModal()} icon={<LockClosedIcon/>} text="Deposit" className="mb-4" />
          <ActionCard onClick={() => setQuitLockModal()} icon={<LockClosedIcon/>} text="Quit Lock" />
        </div>
        <div className="flex flex-col w-full">
          <ActionCard onClick={() => setWithdrawModal()} icon={<LockClosedIcon/>} text="Withdraw" className="mb-4" />
          <ActionCard onClick={() => setExtendLockModal()} icon={<LockClosedIcon/>} text="Extend Lock" />
        </div>
      </div>
    </VaultCard>
  )
