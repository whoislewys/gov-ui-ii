...
import { SNAPSHOT_SPACE_ID_GOERLI } from "./snapshot";
import { getProposals } from "./getProposals";
import {TESTNET_CHAIN_ID} from "./chainIds";
...
(async function () {
  try {
    ...
    const newProposals = await getProposals(provider, coreVotingContract, snapshotIdsByProposalId, targetsByProposalId, callDatasByProposalId, forumLinksByProposalId, TESTNET_CHAIN_ID, []);

    // NOTE: this WILL break if the snapshot id is not found
    const newGscProposals = await getProposals(provider, gscCoreVotingContract, gscSnapshotIdsByProposalId, gscTargetsByProposalId, gscCallDatasByProposalId, gscForumLinksByProposalId,  TESTNET_CHAIN_ID, []);
    ...
    fs.writeFileSync(
      "proposals/src/proposals/testnet.proposals.json",
      proposalsJsonString,
    );
    ...
    fs.writeFileSync(
      "proposals/src/proposals/testnet-gsc.proposals.json",
